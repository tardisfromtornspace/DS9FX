...
pVer = pDS9FXVer.DS9FXPrintableVersion

mode = Foundation.MutatorDef("DS9FX " + pVer)
ET_EVENT = App.UtopiaModule_GetNextEventType()

for k, d in DS9FXSets.dAlphaQuadrantSets.items():
    Foundation.SystemDef(k, 1, dict={"modes": [mode]})
...
class DS9FXMissionStart(Foundation.TriggerDef):
    ...
    def __call__(self, pObject, pEvent, dict={}):
        if not DS9FXMutatorFunctions.CanStart():
            return
...
class DS9FXPreWormholeEntry(Foundation.TriggerDef):
    def __init__(self, name, eventKey, dict={}):
        Foundation.TriggerDef.__init__(self, name, eventKey, dict)
    ...
    def __call__(self, pObject, pEvent):
        if not DS9FXMutatorFunctions.CanStart():
            return
        DS9FXMutatorFunctions.PreWormholeEntryHandling(pObject, pEvent)
        if pObject and pEvent:
            pObject.CallNextHandler(pEvent)
...
DS9FXPlayerShipCreated('DS9FX Player Ship Created', Foundation.TriggerDef.ET_FND_CREATE_PLAYER_SHIP,
                       dict={'modes': [mode]})
...
DS9FXAppSubsystemDestroyed('DS9FX App Subsystem Destroyed', DS9FXGlobalEvents.ET_APP_SUBSYSTEM_DESTROYED,
                           dict={'modes': [mode]})
...
DS9FXForceMissionPlaying('DS9FX Force Mission Playing', DS9FXGlobalEvents.ET_FORCE_MISSION_PLAYING,
                         dict={'modes': [mode]})

DS9FXStopForcingMissionPlaying('DS9FX Stop Forcing Mission Playing', DS9FXGlobalEvents.ET_STOP_FORCING_MISSION_PLAYING,
                               dict={'modes': [mode]})
...
DS9FXLifeSupportShipRecrewed('DS9FX Life Support Ship Recrewed', DS9FXGlobalEvents.ET_SHIP_RECREWED,
                             dict={'modes': [mode]})

DS9FXLifeSupportShipDeadInSpace('DS9FX Life Support Ship Dead In Space', DS9FXGlobalEvents.ET_SHIP_DEAD_IN_SPACE,
                                dict={'modes': [mode]})

DS9FXLifeSupportShipReactivated('DS9FX Life Support Ship Reactivated', DS9FXGlobalEvents.ET_SHIP_REACTIVATED,
                                dict={'modes': [mode]})

DS9FXLifeSupportShipTakenOver('DS9FX Life Support Ship Taken Over', DS9FXGlobalEvents.ET_SHIP_TAKEN_OVER,
                              dict={'modes': [mode]})

DS9FXLifeSupportCombatEffectiveness('DS9FX Life Support Combat Effectiveness',
                                    DS9FXGlobalEvents.ET_COMBAT_EFFECTIVENESS, dict={'modes': [mode]})

DS9FXLifeSupportCombatEffectivenessAdjusted('DS9FX Life Support Combat Effectiveness Adjusted',
                                            DS9FXGlobalEvents.ET_COMBAT_EFFECTIVENESS_ADJUSTED, dict={'modes': [mode]})

DS9FXLifeSupportCustomDamage('DS9FX Life Support Custom Damage', DS9FXGlobalEvents.ET_CUSTOM_DAMAGE,
                             dict={'modes': [mode]})
...
DS9FXAppMusicConditionChanged('DS9FX App Music Condition Changed', DS9FXGlobalEvents.ET_APP_MUSIC_CONDITION_CHANGED,
                              dict={'modes': [mode]})
...
DS9FXPreWormholeEntry('DS9FX Pre Wormhole Entry', DS9FXGlobalEvents.ET_PRE_WORMHOLE_ENTRY, dict={'modes': [mode]})

DS9FXPrintConsole(dict={"modes": [mode]})

App.g_kKeyboardBinding.BindKey(App.WC_CAPS_P, App.TGKeyboardEvent.KS_NORMAL, ET_EVENT,
                               App.KeyboardBinding.GET_INT_EVENT, 5000)
