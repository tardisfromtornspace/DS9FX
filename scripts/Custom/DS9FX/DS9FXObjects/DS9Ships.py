...
def DS9SetShips():
    ...
    if DisableLocalForces.ShouldEnforceLocalForces():
        return
...
def ResetAI(pSet):
    ...
    DS9Set = DS9.GetSet()
    if DS9Set:
        if DS9Set.GetName() == pSet.GetName():
            reload(DS9FXSavedConfig)
            if DS9FXSavedConfig.FederationSide == 1:
                if DS9FXSavedConfig.ExcaliburSelection == 1:
                    import Custom.DS9FX.DS9FXAILib.DS9FXExcalAI
                ...
                if DS9FXSavedConfig.LakotaSelection == 1:
                    ...
                    if Lakota:
                        ...
                        pLakota.ClearAI()
                        pLakota.SetAI(Custom.DS9FX.DS9FXAILib.DS9FXLakotaAI.CreateAI(pLakota))
                from Custom.DS9FX.DS9FXLib import DS9FXLauncher
                l = DS9FXLauncher.lNames
                for s in l:
                    pShip = MissionLib.GetShip(s, DS9Set)
                    if pShip:
                        from Custom.DS9FX.DS9FXAILib import DS9FXGenericStaticFriendlyAI
                        pShip.ClearAI()
                ...
                for s in l:
                    pShip = MissionLib.GetShip(s, DS9Set)
                    if pShip:
                        from Custom.DS9FX.DS9FXAILib import DS9FXGenericStaticEnemyAI
                        pShip.ClearAI()
                        pShip = App.ShipClass_Cast(pShip)
