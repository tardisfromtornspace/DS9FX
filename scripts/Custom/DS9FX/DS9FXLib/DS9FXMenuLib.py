...
def CreateSliderbar(pName, eType, fValue=0.0, fRangeMin=0.0, fRangeMax=1.0, fKeyInterval=0.01, fMarkerValue=1.0,
                    fWidth=0.25, fHeight=0.04):
...
def AddKeyBind(sKeyName, sFunction, iEventInt=0, sGroup="General", eType=App.KeyboardBinding.GET_INT_EVENT):
    ...
    Foundation.g_kKeyBucket.AddKeyConfig(
        Foundation.KeyConfig(sKeyName, sKeyName, ET_KEY_EVENT, eType, iEventInt, sGroup, dict={"modes": [mode]}))
...
def ColorizeButton(pButton, kNormR=0.8, kNormG=0.6, kNormB=0.8, kNormA=1.0, kHighR=0.92, kHighG=0.76, kHighB=0.92,
                   kHighA=1.0, kDisR=0.25, kDisB=0.25, kDisG=0.25, kDisA=1.0):
    kNorm = App.TGColorA()
    kHigh = App.TGColorA()
    kDis = App.TGColorA()
