...
def CreateEntries(pObject, pEvent):
    ...
    pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Mission: USS Odyssey"),
    ...
    pText = App.TGParagraph_CreateW(
        App.TGString("-Go to Gamma Quadrant\n-Wait until Odyssey is destroyed\n-Run back to Alpha Quadrant"),
        pObjectivesWindow.GetMaximumInteriorWidth(), None, '', pObjectivesWindow.GetMaximumInteriorWidth(),
    ...
    pText = App.TGParagraph_CreateW(App.TGString(
        "Your mission is to go with the USS Odyssey to the Gamma Quadrant and rescue Commander Sisko from this new enemy. Good hunting!"),
...
def MissionPlacement(pObject, pEvent):
    ...
    if (pSet.GetName() == "GammaQuadrant1"):
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".OdysseyDamage",
                               App.g_kUtopiaModule.GetGameTime() + 35, 0, 0)
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".RemoveSupport",
                               App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)
...
def OdysseyDamage(pObject, pEvent):
    ...
    MissionLib.DamageShip('USS Odyssey', fMinPercent=0.25, fMaxPercent=0.30, bDisableNothing=0, bDisableWeapons=0,
                          bDisableEngines=1, bDisableSpecial=1)
    ...
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".AISwap",
                           App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)
...
def RemoveMissionShips():
    ships = ["Ship 1", "Ship 2", "Ship 3", "USS Odyssey"]
