...
def CreateEntries(pObject, pEvent):
    ...
    pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize",
                                               App.TGString("Mission: Plant the minefield"), 0.0, 0.0, None, 1, 0.30,
                                               0.30, kColor)
    ...
    pText = App.TGParagraph_CreateW(App.TGString(
        "-Be sure to be in 10 km range before you plant mines\n-Plant 26 mines\n-Survive attack\n-In the end warp out in any direction"),
    ...
    pText = App.TGParagraph_CreateW(App.TGString(
        "Captain, we are loosing at peace time. We have to stop the Dominion convoys which are coming through the wormhole constantly. It's up to you to place a cloaked minefield, you don't have a lot of time before the Dominion and Cardassian fleets arrive from Cardassia. Good luck, Captain. We are all counting on you! This is most likely the first battle of what will people one day call the Dominion Wars, let us pray that we can win."),
...
def LayMines():
    ...
    pTimer = MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".MineLayingCheck",
...
def MineLayingCheck(pObject, pEvent):
    ...
    if DistanceCheck(pDS9FXWormhole) < 50:
        if iMinesLayed == 12:
            ...
            App.g_kEventManager.AddBroadcastPythonFuncHandler(App.ET_WARP_BUTTON_PRESSED, pMission,
                                                              __name__ + ".EvacDS9")
            return
