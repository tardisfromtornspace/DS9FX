...
def CreateEntries(pObject, pEvent):
    ...
    pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize",
                                               App.TGString("Mission: Once More Unto The Breach"), 0.0, 0.0, None, 1,
                                               0.30, 0.30, kColor)
    ...
    pButtonWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize", App.TGString("Please Select"), 0.0,
                                                 0.0, None, 1, 0.60, 0.08, App.g_kMainMenuBorderMainColor)
    ...
    pText = App.TGParagraph_CreateW(App.TGString(
        "The Dominion just confirmed that their taskforce was destroyed on the far side of the wormhole by the rogues. So it is up to you once again to head to the Gamma Quadrant and put an end to the threat."),
...
def MissionInitiate(pObject, pEvent):
    ...
    ObjectiveShipList = ["Renegade 1", "Renegade 2", "Renegade 3", "Renegade 4", "Renegade 5", "Renegade 6",
                         "Renegade 7", "Renegade 8"]
...
def RemoveMissionShips():
    ships = ["USS Moscow", "USS Budapest", "USS Legacy", "USS Morning Star", "Renegade 1", "Renegade 2", "Renegade 3",
             "Renegade 4", "Renegade 5", "Renegade 6", "Renegade 7", "Renegade 8"]
...
def BackToDS9(pObject, pEvent):
    ...
    if (pSet.GetName() == "DeepSpace91"):
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".RemoveSupportShips",
                               App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)

        reload(DS9FXSavedConfig)
        if DS9FXSavedConfig.CompletionVid == 1:
            MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".PlayVictoryVid",
                                   App.g_kUtopiaModule.GetGameTime() + 15, 0, 0)
