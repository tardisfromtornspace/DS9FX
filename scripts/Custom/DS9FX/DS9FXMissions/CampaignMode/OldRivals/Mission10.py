...
pShipType = None
lAllies = ["USS Soyuz", "USS Rising Sun", "USS Elevation", "USS Ridger", "USS Arthur", "IKS Tagh", "IKS Kho'chi",
           "IKS D'ghor", "IKS Khalil"]
...
def CreateFriendlies():
    global lAllies
    lTypes = [DS9FXShips.Akira, DS9FXShips.Nebula, DS9FXShips.Galaxy, DS9FXShips.Nebula, DS9FXShips.Akira,
              DS9FXShips.Vorcha, DS9FXShips.Vorcha, DS9FXShips.Ktinga, DS9FXShips.BirdOfPrey]
    pPlayer = MissionLib.GetPlayer()
    pSet = pPlayer.GetContainingSet()
...
def MissionHandler(pObject, pEvent):
    ...
    if pSet.GetName() == "GammaQuadrant1":
        BorgText()
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".DeleteDominion",
                               App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".BorgArrival",
                               App.g_kUtopiaModule.GetGameTime() + 80, 0, 0)
        App.g_kEventManager.RemoveBroadcastHandler(App.ET_ENTERED_SET, pMission, __name__ + ".MissionHandler")
...
def MissionEnd(pObject, pEvent):
    ...
    if pSet.GetName() == "DeepSpace91":
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".RemoveShips",
                               App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)
        SaveProgress()
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".PlayMovie",
                               App.g_kUtopiaModule.GetGameTime() + 25, 0, 0)
        try:
            App.g_kEventManager.RemoveBroadcastHandler(App.ET_OBJECT_EXPLODING, pMission, __name__ + ".PlayerExploding")
            App.g_kEventManager.RemoveBroadcastHandler(App.ET_ENTERED_SET, pMission, __name__ + ".MissionHandler")
            App.g_kEventManager.RemoveBroadcastHandler(App.ET_OBJECT_EXPLODING, pMission, __name__ + ".ObjectExploding")
...
def RemoveMissionShips():
    ships = ["USS Soyuz", "USS Rising Sun", "USS Elevation", "USS Ridger", "USS Arthur", "IKS Tagh", "IKS Kho'chi",
             "IKS D'ghor", "IKS Khalil", "Borg Sphere", "Borg Tactical Cube"]
