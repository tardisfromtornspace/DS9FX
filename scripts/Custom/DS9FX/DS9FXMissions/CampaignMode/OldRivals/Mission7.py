...
def SetupMissionSpecific():
    ...
    for s in lDestroyed:
        SetupCondition(s)
        CreateWreckage(s)
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".DamageShip",
...
def SetupCondition(s):
    ...
    pMission = pEpisode.GetCurrentMission()
    pPlayer = MissionLib.GetPlayer()
    DistanceCheckCondition = App.ConditionScript_Create("Conditions.ConditionInRange", "ConditionInRange", 150,
                                                        pPlayer.GetName(), s)
...
def InRange(bInRange):
    ...
    if len(lDestroyed) == 0:
        ...
        NotifyDone()
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".ScanComplete",
                               App.g_kUtopiaModule.GetGameTime() + 30, 0, 0)
    else:
        MoreToScan()
...
def ScanComplete(pObject, pEvent):
    PowerDownNow()
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".WarpInTimer",
...
def WarpInTimer(pObject, pEvent):
    ...
    if not bCreated:
        ...
        if iTime > 60:
            ...
            pEpisode = pGame.GetCurrentEpisode()
            pMission = pEpisode.GetCurrentMission()
            MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".MissionSuccessText",
                                   App.g_kUtopiaModule.GetGameTime() + 5, 0, 0)
...
def ProbePlayerPowerSettings():
    ...
    if not pPlayer:
        return 0
    lLevels = [pPlayer.GetImpulseEngineSubsystem(), pPlayer.GetShields(), pPlayer.GetSensorSubsystem(),
               pPlayer.GetPhaserSystem(), pPlayer.GetTorpedoSystem(), pPlayer.GetPulseWeaponSystem()]
    for pSys in lLevels:
        if pSys:
            if pSys.IsOn():
                return 1
...
def RemoveMissionShips():
    ships = ["Borg Tactical Cube", "Wreckage 1", "Wreckage 2", "Wreckage 3", "Wreckage 4", "Wreckage 5"]
