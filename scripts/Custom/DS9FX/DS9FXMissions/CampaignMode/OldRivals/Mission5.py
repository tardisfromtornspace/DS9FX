...
def InOrbit(bInOrbit):
    ...
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".TransferComplete",
...
def TransferComplete(pObject, pEvent):
    ...
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".WarpIn",
                           App.g_kUtopiaModule.GetGameTime() + 20, 0, 0)
...
def WarpIn(pObject, pEvent):
    ...
    CreateWarship()
    NotifyPresence()
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".StartTheClock",
...
def StartTheClock(pObject, pEvent):
    ...
    if iTime >= 360:
        try:
            ...
            pass
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".MissionFailedText",
                               App.g_kUtopiaModule.GetGameTime() + 5, 0, 0)
        DS9FXGlobalEvents.Trigger_Stop_Forcing_Mission_Playing(MissionLib.GetPlayer())
        DS9FXGlobalEvents.Trigger_DS9FX_Mission_End(MissionLib.GetPlayer(), pName)
        return
...
def RemoveMissionShips():
    ships = ["Unknown Dominion Warship"]
