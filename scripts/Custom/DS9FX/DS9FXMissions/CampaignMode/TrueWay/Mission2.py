...
def HandleMissionDetails():
    ...
    if iProgress == 1:
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".ScanOver", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(40, 35), 0, 0)
    elif iProgress == 2:
        ShowMessage("Scanning system...")
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".ScanOver", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(35, 35), 0, 0)
    elif iProgress == 3:
        PopulateSystem()
        ShowMessage("Scanning system...")
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".ScanOver", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(30, 35), 0, 0)
...
def ScanOver(pObj, pEvent):
    ...
    if iProgress == 3:
        ...
        MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".RunAway", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(20, 30), 0, 0)
...
def InScanningRange(bInRange):
    ...
    pMission = pEpisode.GetCurrentMission()     
    MissionLib.StopCallingFunctionWhenConditionChanges(__name__, "InScanningRange")
    ShowMessage("Nothing unusual on the station, but according to these readings something\nwas beamed off Empok Nor a few minutes ago.\n\nWe should return to DS9 and report this.")
