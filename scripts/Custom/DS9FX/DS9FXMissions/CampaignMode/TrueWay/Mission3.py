...
def SetupSet():
    ShowMessage("We should enter orbit of Trivas 4. We'll need to keep our shields and weapons offline in order to avoid detection.")
    pGame = App.Game_GetCurrentGame()
    pEpisode = pGame.GetCurrentEpisode()
    pMission = pEpisode.GetCurrentMission()
    pCondition = App.ConditionScript_Create("Conditions.ConditionPlayerOrbitting", "ConditionPlayerOrbitting", "Trivas 4")
...
def CreateEmpokNor():
    ...
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".RemoveCrew", App.g_kUtopiaModule.GetGameTime() + 10, 0, 0)
...
def InOrbit(bInOrbit):
    ...
    ShowMessage(message)
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".WarbirdEntering", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(10, 35), 0, 0)

def ProbePlayerPowerSettings():
    pPlayer = MissionLib.GetPlayer()
    if not pPlayer:
        return 0
    lLevels = [pPlayer.GetShields(), pPlayer.GetPhaserSystem(), pPlayer.GetTorpedoSystem(), pPlayer.GetPulseWeaponSystem()]
    for pSys in lLevels:
        if pSys:
            if pSys.IsOn():
                return 1
...
def WarbirdEntering(pObject, pEvent):
    ...
    CreateWarbird()
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".TransportRelated", App.g_kUtopiaModule.GetGameTime() + GetRandomNumber(10, 35), 0, 0)
...
def TransportRelated(pObject, pEvent):
    ShowMessage("I'm scanning Empok Nor. I'm detecting plasma torpedoes, phaser batteries, and shield generators. We should confiscate the shipment and report this to DS9!")
    ...
    pEpisode = pGame.GetCurrentEpisode()
    pMission = pEpisode.GetCurrentMission()         
    pCheck = App.ConditionScript_Create("Conditions.ConditionInRange", "ConditionInRange", 120, pPlayer.GetName(), "Empok Nor")
