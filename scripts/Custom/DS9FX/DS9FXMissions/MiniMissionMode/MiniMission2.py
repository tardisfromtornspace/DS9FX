...
def CreateEntries(pObject, pEvent):
    ...
    pIconWindow = App.STStylizedWindow_CreateW("StylizedWindow", "NoMinimize",
                                               App.TGString("Mission: Anomalous Readings"), 0.0, 0.0, None, 1, 0.30,
                                               0.30, kColor)
    ...
    pText = App.TGParagraph_CreateW(
        App.TGString("-Get in close range to scan the comet\n-Scan the comet for 1 minute\n-See what happens next"),
    ...
    pText = App.TGParagraph_CreateW(App.TGString(
        "We are getting some strange readings from the Comet Alpha. You are required to scan the comet and check out the readings. Who knows, you might even discover something new Captain..."),
...
def MissionInitiate(pObject, pEvent):
    ...
    DistanceCheckCondition = App.ConditionScript_Create("Conditions.ConditionInRange", "ConditionInRange", 40,
                                                        pPlayer.GetName(), pComet)
...
def ScanTimer():
    ...
    MissionLib.CreateTimer(DS9FXMenuLib.GetNextEventType(), __name__ + ".ScanConfirm",
...
def WinSequence(pPane):
    ...
    pAction = LineAction(
        "It would appear that the Cardassians had some sort of \nlistening device on the Comet! We have neutralized it. \nMission Completed!",
        pPane, 6, 10, 12)
    pSequence.AddAction(pAction, None, 10)
    pAction = App.TGScriptAction_Create(__name__, "KillPane")
    pSequence.AppendAction(pAction, 0.1)
...
def RemoveMissionShips():
    ships = ["Galor 1", "Galor 2", "Galor 3", "Galor 4", "Galor 5"]
