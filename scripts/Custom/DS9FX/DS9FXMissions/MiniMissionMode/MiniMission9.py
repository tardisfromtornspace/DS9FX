...
def MissionInitiate():
    ...
    App.g_kEventManager.AddBroadcastPythonFuncHandler(DS9FXGlobalEvents.ET_SHIP_DEAD_IN_SPACE, pMission,
                                                      __name__ + ".ObjectExploding")
    App.g_kEventManager.AddBroadcastPythonFuncHandler(DS9FXGlobalEvents.ET_REPAIR_SHIP, pMission,
                                                      __name__ + ".HandleDocking")
...
def PlayerExploding(pObject, pEvent):
    ...
    if (pShip.GetObjID() == pPlayer.GetObjID()):
        FailedTxt()
        try:
            App.g_kEventManager.RemoveBroadcastHandler(DS9FXGlobalEvents.ET_REPAIR_SHIP, pMission,
                                                       __name__ + ".HandleDocking")
