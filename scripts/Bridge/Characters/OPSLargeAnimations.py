...
#	print("Place T")
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "EB_seated_t_l")
	pSequence.AddAction(pAction)
	return pSequence
...
def EBPlaceAtL2(pCharacter):
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/EB_L2toT_L.nif", "EB_L2toT_L")
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "EB_L2toT_L")
	pSequence.AddAction(pAction)
	return pSequence
...
def EBPlaceAtL1(pCharacter):
	...
	pSequence = App.TGSequence_Create()
	pAnimNode = pCharacter.GetAnimNode()
	pAction = App.TGAnimPosition_Create(pAnimNode, "EB_L1toT_L")
	pSequence.AddAction(pAction)
	return pSequence
...
#	print("Move to T")
	...
	# From L2 to T
def EBMoveFromL2ToT(pCharacter):
	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/EB_L2toT_L.nif", "EB_L2toT_L")
	pSet = App.g_kSetManager.GetSet("bridge")
	pBridge = pSet.GetObject("bridge")
	...
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimAction_WalkFromLift = App.TGAnimAction_Create(pCharacter.GetAnimNode(), "EB_L2toT_L", 0, 0, 1)
	pSequence.AddAction(pAnimAction_WalkFromLift)
	pSequence.AddAction(App.TGAnimAction_Create(pBridgeNode, "EB_Door_L2", 0, 0), App.TGAction_CreateNull(), 0.125)
	pSequence.AddAction(pAnimAction, pAnimAction_WalkFromLift)
	pSequence.AppendAction(App.CharacterAction_Create(pCharacter, App.CharacterAction.AT_SET_LOCATION_NAME, "EBTactical"))
	pEvent = App.TGIntEvent_Create ()
	pEvent.SetEventType (App.ET_CHARACTER_ANIMATION_DONE)
	...
	# From T to L2
def EBMoveFromTToL2(pCharacter):
 	kAM = App.g_kAnimationManager
	kAM.LoadAnimation ("data/animations/EB_TtoL2_l.nif", "EB_TtoL2_l")
	...
	pCharacter = App.CharacterClass_Cast(pCharacter)
	pSequence = App.TGSequence_Create()
	pAnimAction_WalkToLift = App.TGAnimAction_Create(pCharacter.GetAnimNode(), "EB_TtoL2_l", 0, 0)
	pSequence.AddAction(pAnimAction_WalkToLift)
	fTime = fTime + kAM.GetAnimationLength("EB_TtoL2_l")
	pAnimAction = App.TGAnimAction_Create(pBridgeNode, "EB_Door_L2", 0, 0)
	pSequence.AppendAction(App.CharacterAction_Create(pCharacter, App.CharacterAction.AT_SET_LOCATION_NAME, "EBL2L"))
	pEvent = App.TGIntEvent_Create ()		# Add event to hide character after it gets into the turbolift
	pEvent.SetEventType (App.ET_CHARACTER_ANIMATION_DONE)
	pEvent.SetDestination (pCharacter)
	pEvent.SetInt (App.CharacterClass.CS_HIDDEN)
